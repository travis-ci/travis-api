...
describe Travis::API::V3::Services::Overview::RecentBuildHistory do
  ...
  describe "recent build history on public repository dynamic" do
    before  {
      ...
      Travis::API::V3::Models::Build.create(repository_id: repo.id, created_at: DateTime.now, state: 'passed',  branch_name: branch.name)
      Travis::API::V3::Models::Build.create(repository_id: repo.id, created_at: DateTime.now, state: 'failed',  branch_name: branch.name)
      Travis::API::V3::Models::Build.create(repository_id: repo.id, created_at: DateTime.now, state: 'errored', branch_name: branch.name)
  ...
  describe "recent build history on public repository dynamic, fringe cases" do
    before  {
      Travis::API::V3::Models::Build.where(repository_id: repo.id).each do |build| build.destroy end
      Travis::API::V3::Models::Build.create(repository_id: repo.id, created_at: DateTime.now - 9,  state: 'passed',  branch_name: branch.name)
      Travis::API::V3::Models::Build.create(repository_id: repo.id, created_at: DateTime.now,      state: 'failed',  branch_name: branch.name)
      Travis::API::V3::Models::Build.create(repository_id: repo.id, created_at: DateTime.now - 10, state: 'errored', branch_name: branch.name)
      get("/v3/repo/#{repo.id}/overview/build_history") }
    example { expect(last_response).to be_ok            }
    example { expect(parsed_body).to be == {
      "@type"                => "overview",
