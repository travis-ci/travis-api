describe Travis::API::V3::Services::InsightsProbe::Update, set_app: true, insights_spec_helper: true do
  ...
  context 'unauthenticated' do
    it 'responds 403' do
      patch("/v3/insights_probe/#{probe_id}")
  ...
  context 'authenticated' do
    ...
    let(:probe_data) { { 'notification' => notification, 'probe_id' => probe_id } }
    ...
    before do
      stub_insights_request(:patch, "/probes/#{probe_id}", auth_key: insights_auth_key, user_id: user.id)
    ...
    it 'responds with authenticate result' do
      patch("/v3/insights_probe/#{probe_id}", { notification: notification }, headers)
