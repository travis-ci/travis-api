...
describe Travis::API::V3::Services::BetaMigrationRequest::ProxyCreate, set_app: true do
  ...
  let!(:org1) { FactoryGirl.create(:org_v3, name: "org_1")}
  let!(:org2) { FactoryGirl.create(:org_v3, name: "org_2")}
  let!(:org3) { FactoryGirl.create(:org_v3, name: "org_3")}
  ...
  before do
    valid_org_ids.each do |org_id|
      FactoryGirl.create(:membership, role: "admin", organization_id: org_id, user_id: user.id)
    end
