...
describe Travis::API::V3::Services::BetaMigrationRequests::Create, set_app: true do
  let(:user)  { FactoryGirl.create(:user, login: 'some_beta_user') }
  ...
  let!(:org1) { FactoryGirl.create(:org_v3, name: "org_1", login: "org_1")}
  let!(:org2) { FactoryGirl.create(:org_v3, name: "org_2", login: "org_2")}
  let!(:org3) { FactoryGirl.create(:org_v3, name: "org_3", login: "org_3")}

  let(:valid_org_names) { [org1.login, org2.login, org3.login]}
  let(:valid_orgs) { [org1, org2, org3]}

  let(:invalid_org) { FactoryGirl.create(:org_v3, name: "invalid_org") }

  before do
    Travis.config.applications[:some_app] = { token: 'sometoken', full_access: true }

    valid_orgs.each do |org|
      FactoryGirl.create(:membership, role: "admin", organization_id: org.id, user_id: user.id)
    end
  end
