...
describe Travis::API::V3::Services::SslKey::Find, set_app: true do
  ...
  describe 'not authenticated' do
    before { get("/v3/repo/#{repo.id}/key_pair/default") }
    include_examples 'not authenticated'
  end

  context 'authenticated' do
    describe 'missing repo' do
      before { get("/v3/repo/999999999/key_pair/default", {}, auth_headers) }
    ...
    describe 'existing repo, no key' do
      before do
        repo.key.destroy
        get("/v3/repo/#{repo.id}/key_pair/default", {}, auth_headers)
      end
    ...
    describe 'existing repo, existing key' do
      before { get("/v3/repo/#{repo.id}/key_pair/default", {}, auth_headers) }
      example { expect(last_response.status).to eq 200 }
      example do
        expect(JSON.parse(last_response.body)).to eq(
          '@type' => 'key_pair',
          '@href' => "/v3/repo/#{repo.id}/key_pair/default",
