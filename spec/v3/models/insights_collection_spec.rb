describe Travis::API::V3::Models::InsightsCollection do
  let(:collection) { [ { id: 1, name: 'test1' }, { id: 2, name: 'test2' }, { id: 3, name: 'test3' } ] }
  subject { Travis::API::V3::Models::InsightsCollection.new(collection, collection.count) }
  ...
  describe "#to_sql" do
    ...
    it 'returns placeholder string' do
      expect(subject.to_sql).to eq("insights_query:#{Time.now.to_i}")
    end
  end
end
