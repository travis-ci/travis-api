describe Travis::Api::App::Endpoint::Logs, set_app: true do
  ...
  context do
    ...
    let(:public_migrated_repo) { FactoryGirl.create(:repository, private: false, migrated_at: 1.day.ago) }
    let(:public_migrated_build) { FactoryGirl.create(:build, repository: public_migrated_repo, private: false) }
    ...
    describe 'public mode, .com v2' do
      ...
      describe 'given the current user has a permission on the repository' do
        ...
        it 'responds with public log from .org when job already migrated not restarted' do
          ...
          FactoryGirl.create(:permission, user: user, repository: public_migrated_repo)
