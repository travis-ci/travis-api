...
RSpec.feature 'Search', js: true, type: :feature do
  ...
  let!(:commit)       { create(:commit, committer_name: 'Katrin', committer_email: 'katrin@example.com',
                               author_name: 'Katrin', author_email: 'katrin@example.com') }
  let!(:build)        { create(:build, commit: commit, config: {}) }
  let!(:job)          { create(:job, commit: commit, config: {}) }

  before(:each) { allow_any_instance_of(Services::Job::GetLog).to receive(:call) }
