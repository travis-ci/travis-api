module Travis::API::V3
  class Services::Overview::GetStreakData < Service
    ...
    def run!
      ...
      last_failing_build = Models::Build.where(:repository_id => repo.id, :branch => repo.default_branch_name, :state => 'failed', :event_type => 'push').order("id DESC").first

      fail_id = (last_failing_build != nil) ? last_failing_build.id : 0

      first_build_of_streak = Models::Build.where(:repository_id => repo.id, :branch => repo.default_branch_name, :state => 'passed', :event_type => 'push').where("id > ?", fail_id).order("id ASC").first
