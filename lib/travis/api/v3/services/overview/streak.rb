module Travis::API::V3
  class Services::Overview::Streak < Service
    ...
    def run!
      ...
      first_build_of_streak = Models::Build.where(:repository_id => repo.id, :branch => repo.default_branch_name, :state => 'passed', :event_type => ['push', 'cron']).where("id > ?", fail_id).order("id ASC").select(:created_at).first
