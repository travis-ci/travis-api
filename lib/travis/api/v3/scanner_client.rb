  ...
  class ScannerClient
    ...
    def connection(timeout: 20)
      @connection ||= Faraday.new(url: scanner_url, ssl: { ca_path: '/usr/lib/ssl/certs' }) do |conn|
        conn.headers[:Authorization] = "Token token=\"#{scanner_auth_token}\""
    ...
    def scanner_auth_token
      Travis.config.scanner.auth_token || raise(ConfigurationError, 'No Scanner Auth Token configured!')
    end
