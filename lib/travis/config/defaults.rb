...
module Travis
  class Config < Hashr
    ...
    class << self
      ...
      def yml_token
        ENV['TRAVIS_YML_TOKEN'] || 'secret'
      end

      def yml_auth_key
        ENV['TRAVIS_YML_AUTH_KEY'] || 'abc123'
      end

      def closeio_key
        ENV['TRAVIS_CLOSEIO_KEY'] || 'key'
      end

      def insights_auth_token
        ENV['TRAVIS_INSIGHTS_AUTH_TOKEN'] || 'secret'
      end

      def s3_access_key_id
        ENV['TRAVIS_S3_ACCESS_KEY_ID'] || ''
      end

      def s3_secret_access_key
        ENV['TRAVIS_S3_SECRET_ACCESS_KEY'] || ''
      end

      def pusher_key
        ENV['TRAVIS_PUSHER_KEY'] || 'key'
      end

      def pusher_secret
        ENV['TRAVIS_PUSHER_SECRET'] || 'secret'
      end

      def github_token
        ENV['TRAVIS_GITHUB_TOKEN'] || 'travisbot-token'
      end

      def encryption_key
        ENV['TRAVIS_ENCRYPTION_KEY'] || 'secret' * 10
      end

      def merge_auth_token
        ENV['TRAVIS_MERGE_AUTH_TOKEN'] || 'merge-auth-token'
      end
      ...
      def redis_gatekeeper_url
        ENV['REDIS_GATEKEEPER_URL'] || 'redis://localhost:6379'
      end
    ...
    define  host:                 'travis-ci.org',
            ...
            amqp:                 { username: amqp_username, password: amqp_password, host: 'localhost', prefetch: 1 },
            closeio:              { key: closeio_key },
            gdpr:                 {},
            ...
            log_options:          { s3: { access_key_id: s3_access_key_id, secret_access_key: s3_secret_access_key}},
            s3:                   { access_key_id: s3_access_key_id, secret_access_key: s3_secret_access_key},
            pusher:               { app_id: 'app-id', key: pusher_key, secret: pusher_secret },
            sidekiq:              { namespace: 'sidekiq', pool_size: 1 },
            smtp:                 {},
            email:                {},
            github:               { api_url: 'https://api.github.com', token: github_token },
            ...
            merge:                { auth_token: merge_auth_token, api_url: 'https://merge.localhost' },
            ...
            billing:              {},
            vcs:                  {},
            yml:                  { url: 'https://yml.travis-ci.org', token: yml_token, auth_key: yml_auth_key },
            ...
            scanner:              {},
            insights:             { endpoint: 'https://insights.travis-ci.dev/', auth_token: insights_auth_token }
