  ...
  module Services
    class FindCaches < Base
      ...
      class S3Wrapper
        ...
        def branch
          repository.vcs_type == 'GithubRepository' ? s3_object.key[%r{^\d+/(.*)/[^/]+$}, 1] : s3_object.key[%r{^(.*)/(.*)/[^/]+$}, 2]
        end
      ...
      class GcsWrapper
        ...
        def branch
          repository.vcs_type == 'GithubRepository' ? cache_object.name[%r{^\d+/(.*)/[^/]+$}, 1] : cache_object.name[%r{^(.*)/(.*)/[^/]+$}, 2]
        end
