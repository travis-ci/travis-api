    ...
    if settings.valid?
      settings.attributes.except(*not_in_api).each do |setting_name, setting_value|
        if current_settings.attributes[setting_name] != setting_value
          ...
          Models::Audit.create!(owner: current_user, change_source: 'admin-v2', source: @repository, source_changes: { settings: { :"#{setting_name}" => { before: current_settings.attributes[setting_name], after: setting_value } } })
          if response.success?
